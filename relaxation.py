...
def simplex(matrix):
    ...
    while state:
        ...
        masked_matrix = matrix[:np.where(matrix.T[-1] > 0)[1][-1] + 1]
        pivot_row_division = np.divide(masked_matrix.T[-1], masked_matrix.T[pivot_column])
        ...
        pivot_element = {'column': pivot_column, 'row': pivot_row_index}
        ...
        matrix = np.insert(check_matrix, pivot_row_index, pivot_row, 0)
